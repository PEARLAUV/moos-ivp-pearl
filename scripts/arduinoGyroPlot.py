...
class serialPlot:
    def __init__(self, serialPort='/dev/ttyACM0', serialBaud=115200, plotLength=100, dataNumBytes=2, numPlots=1):
    ...
    def getSerialData(self, frame, lines, lineValueText, lineLabel, timeText):
        ...
        for i in range(self.numPlots):
            ...
            avg = round(statistics.mean(self.data[i]), 8)
            lineValueText[i].set_text('[' + lineLabel[i] + '] = ' + str(avg))
...
def main():
    ...
    ax.grid(True)
    ax.set_title('NXP IMU Gyroscope')
    ax.set_xlabel("Time")
    ax.set_ylabel("Gyroscope Output [rad/s]")
